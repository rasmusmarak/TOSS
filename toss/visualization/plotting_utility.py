...
def distance_deviation_over_time(n_spacecraft, r_bound_min, r_bound_max, positions, timelist):
    ...
    if n_spacecraft > 1:
        colors = ["blue", "darkorange", "brown", "purple"]
        ...
        for spacecraft_idx in range(0, len(positions_list)):
            ...
            ax.plot(timelist, distances, c=colors[spacecraft_idx]) #, label='Trajectory '+str(spacecraft_idx+1)
    else:
    ...
    ax.axhline(y = r_bound_min, color = 'r', linewidth=2.0, linestyle = 'dashed', label = "Inner bounding sphere")

    # Plot outer bounding sphere radius
    ax.axhline(y = r_bound_max, color = 'g', linewidth=2.0, linestyle = '--', label = "Outer bounding sphere")

    # Change ticks aspect ratio from meters to kilometers
    m2km = lambda y, _: f'{y/1000:g}'
    ax.yaxis.set_major_formatter(m2km)

    s2days = lambda x, _: f'{np.ceil((x/3600)/24):g}'
    ...
    ax.set_ylabel('Distance [km]', fontsize=16, labelpad=8)
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig('distance_deviation_over_time.pdf') # dpi=600

    


def fitness_over_time(fitness_list, coverage_list, close_dist_penalty_list, far_dist_penalty_list, time_list):
    ...
    # Plot fitness over generations
    figure, ax = plt.subplots(figsize=(5, 5), dpi=150)
    ax.plot(time_list, fitness_list, label='Fitness function', c="red")
    ax.plot(time_list, coverage_list, label='Coverage', c="blue")
    ax.plot(time_list, close_dist_penalty_list, label='Close distance penalty', c="darkorange")
    ax.plot(time_list, far_dist_penalty_list, label='Far distance penalty', c="green")
    ...
    ax.tick_params(axis='x', labelsize=16)
    ax.tick_params(axis='y', labelsize=16)
    ax.set_xlabel('Time [days]', fontsize=16, labelpad=10)
    ax.set_ylabel('Fitness value', fontsize=16, labelpad=5)
...
def fitness_over_generations(fitness_list, number_of_generations):
    ...
    ax.tick_params(axis='x', labelsize=16)
    ax.tick_params(axis='y', labelsize=16)
    ax.set_xlabel('Number of generation', fontsize=16, labelpad=10)
    ax.set_ylabel('UDP fitness value', fontsize=16, labelpad=6)
...
def plot_UDP_3D(args, positions, maneuver_positions, maneuver_unit_vectors, plot_mesh, plot_trajectory, plot_control, plot_risk_zone, plot_measurement_sphere, view_angle, measurement_radius):
    ...
    if plot_trajectory:
        ...
        for i in range(0,len(positions[0,:]), n_pos):
            ...
            ax.scatter(x[0], y[0], z[0], marker='x', c='black', label="Initial position")
    ...
    #ax.quiver(0, 0, 0, 5.551115123125783e-17, -0.898794046299167, 0.4383711467890775, color='r', length=7000) #arrow_length_ratio
    #ax.plot([], [], [], color="r", label="Thrust direction")
    ...
    if plot_risk_zone:
        ...
        r = args.problem.radius_outer_bounding_sphere
        (x, y, z) = drawSphere(r)
...
def plot_UDP_2D(args, positions, maneuver_positions, maneuver_unit_vectors, plot_mesh, plot_trajectory, plot_control, plot_risk_zone, plot_measurement_sphere, view_angle, measurement_radius):
    """plot_trajectory plots the satellite trajectory.
    Args:
        args (dotmap.DotMap): Dotmap dictionary containing info on mesh and bounding spheres.
        positions (np.ndarray): (3xN) Array containing N positions (cartesian frame) along the computed trajectory.
    ...
    if plot_trajectory:
        ...
        for i in range(0,len(positions[0,:]), n_pos):
            ...
            # Plot square marker at starting position
            ax.scatter(x[0], y[0], marker='s', c='black')

            # Plot pointer at final position
            ax.scatter(x[-1], y[-1], marker='o', c='black')

            # Plot trajectory
            ax.plot(x, y, label='Trajectory '+str(id))
            ax.legend()
            id += 1
    ...
    if plot_control:
        for maneuver_idx in range(0, len(maneuver_unit_vectors[0,:])):
            ...
            ux = maneuver_unit_vectors[0,maneuver_idx]
            uy = maneuver_unit_vectors[1,maneuver_idx]
            ax.quiver([x, y], -ux, -uy, color='r', scale=2500) #arrow_length_ratio
    ...
    if plot_risk_zone:
        ...
        drawObject = Circle([0,0], radius=args.problem.radius_inner_bounding_sphere, fill=False, color="r")
        ax.add_patch(drawObject)

        drawObject = Circle([0,0], radius=args.problem.radius_outer_bounding_sphere, fill=False, color="green")
        ax.add_patch(drawObject)
    ...
    #plt.show()
    plt.savefig('D2_trajectory_plot.png') # fixed_body_frame_trajectory_plot # trajectory_plot
