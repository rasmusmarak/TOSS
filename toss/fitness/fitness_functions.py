...
def get_fitness(chosen_fitness_function: FitnessFunctions, args, positions: np.ndarray, velocities:np.ndarray, timesteps: np.ndarray):
    ...
    if chosen_fitness_function == FitnessFunctions.TargetAltitudeDistance:
        ...
        return covered_space(args.problem.number_of_spacecrafts, args.body.spin_axis, args.body.spin_velocity, args.problem.radius_inner_bounding_sphere, args.problem.radius_outer_bounding_sphere, positions, velocities, timesteps, args.problem.tensor_grid_r, args.problem.tensor_grid_theta, args.problem.tensor_grid_phi, args.problem.bool_tensor, args.problem.tensor_weights)
    
    elif chosen_fitness_function == FitnessFunctions.CoveredSpaceCloseDistancePenaltyFarDistancePenalty:
        return covered_space_close_distance_penalty_far_distance_penalty(args.problem.number_of_spacecrafts, args.body.spin_axis, args.body.spin_velocity, args.problem.radius_inner_bounding_sphere, args.problem.radius_outer_bounding_sphere, positions, velocities, timesteps, args.problem.penalty_scaling_factor, args.problem.tensor_grid_r, args.problem.tensor_grid_theta, args.problem.tensor_grid_phi, args.problem.bool_tensor, args.problem.tensor_weights)
...
def covered_space(number_of_spacecrafts: int, spin_axis: np.ndarray, spin_velocity: float, radius_inner_bounding_sphere: float, radius_outer_bounding_sphere: float, positions: np.ndarray, velocities: np.ndarray, timesteps: np.ndarray, r: np.ndarray, theta: np.ndarray, phi: np.ndarray, bool_tensor: np.ndarray, tensor_weights: np.ndarray) -> float:
    ...
    """
    visited_space_ratio = compute_space_coverage(number_of_spacecrafts, spin_axis, spin_velocity, positions, velocities, timesteps, radius_inner_bounding_sphere, radius_outer_bounding_sphere, r, theta, phi, bool_tensor, tensor_weights)
...
def covered_space_close_distance_penalty_far_distance_penalty(number_of_spacecrafts: int, spin_axis: np.ndarray, spin_velocity: float, radius_inner_bounding_sphere: float, radius_outer_bounding_sphere: float, positions: np.ndarray, velocities: np.ndarray, timesteps: np.ndarray, penalty_scaling_factor: float, r: np.ndarray, theta: np.ndarray, phi: np.ndarray, bool_tensor: np.ndarray, tensor_weights: np.ndarray) -> float:
    ...
    coverage = covered_space(number_of_spacecrafts, spin_axis, spin_velocity, radius_inner_bounding_sphere, radius_outer_bounding_sphere, positions, velocities, timesteps, r, theta, phi, bool_tensor, tensor_weights)
